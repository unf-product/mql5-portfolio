...
### 2. MA-Cross Notifier (MTF_BollingerBands)
#### 🔹 実装機能・特徴
* **マルチタイムフレーム（MTF）描画**
  5分足や15分足といった短期チャート上に、1時間足や4時間足などの長期ボリンジャーバンドのラインを「階段状」に正確にプロットします。
* **主要3ラインの完全サポート**
  * センターライン（移動平均線）
  * アッパーバンド（+$2\sigma$）
  * ロワーバンド（-$2\sigma$）
* **ビットコイン（BTCUSD）等のボラティリティに対応**
  10万ドルを超える高価格帯の銘柄でも、配列のインデックス崩れやメモリバーストを起こさずに生価格を正確に追従します。
  
#### 📸 動作確認・エビデンス
- **検証環境**: MT5 Build環境にて動作確認済み
- **検証通貨ペア**: BTCUSD (15分足・リアルタイム相場にて検証)

**▼ チャート上への描画**
![](product/MTF_BollingerBands/MTF_BollingerBands-execute.png)
